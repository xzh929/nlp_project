...
class TorchtextSentenceDataset(Dataset):
    ...
    def __init__(self, data_path, sentence_field, label_field):
        ...
        self.label_dict = getlabeldict(r"data/name.txt")
        self.label_list_id = [self.label_dict[i] for i in self.label_list]

        for index in trange(len(self.sentence_list)):
            examples.append(Example.fromlist([self.sentence_list[index], self.label_list_id[index]], fields))
